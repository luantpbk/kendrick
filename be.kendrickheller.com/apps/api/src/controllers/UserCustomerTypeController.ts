...
import { UserCustomerTypeService } from '../services/UserCustomerTypeService';
import { ErrorResponseDto } from '@kendrickheller/core';

export class UserCustomerTypeController {
    public static async getAll(req: Request, res: Response) {
        try {
            ...
            const items = await UserCustomerTypeService.getAll(pSize, pPage);
    ...
    public static async getById(req: Request, res: Response) {
        try {
            const { id } = req.params;
            const item = await UserCustomerTypeService.getById(Number(id));
    ...
    public static async create(req: Request, res: Response) {
        try {
            const item = await UserCustomerTypeService.create(req.body);
            const itemResponse = JSON.parse(JSON.stringify(item, (key, value) =>
                typeof value === 'bigint' ? value.toString() : value
            ));
    ...
    public static async update(req: Request, res: Response) {
        try {
            const { id } = req.params;
            const item = await UserCustomerTypeService.update(Number(id), req.body);
            const itemResponse = JSON.parse(JSON.stringify(item, (key, value) =>
                typeof value === 'bigint' ? value.toString() : value
            ));
            res.json(itemResponse);
    ...
    public static async delete(req: Request, res: Response) {
        try {
            const { id } = req.params;
            await UserCustomerTypeService.delete(Number(id));
            res.json(true);
        } catch (error: any) {
            res.status(500).json(new ErrorResponseDto(undefined, error.message));
        }
