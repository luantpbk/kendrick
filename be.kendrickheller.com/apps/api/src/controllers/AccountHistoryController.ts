...
export class AccountHistoryController {
    public static async getAll(req: Request, res: Response) {
        try {
            ...
            const items = await AccountHistoryService.getAll(pSize, pPage);
    ...
    public static async getById(req: Request, res: Response) {
        try {
            const { id } = req.params;
            const item = await AccountHistoryService.getById(Number(id));
    ...
    public static async create(req: Request, res: Response) {
        try {
            const item = await AccountHistoryService.create(req.body);
            const itemResponse = JSON.parse(JSON.stringify(item, (key, value) =>
                typeof value === 'bigint' ? value.toString() : value
            ));
    ...
    public static async update(req: Request, res: Response) {
        try {
            const { id } = req.params;
            const item = await AccountHistoryService.update(Number(id), req.body);
            const itemResponse = JSON.parse(JSON.stringify(item, (key, value) =>
                typeof value === 'bigint' ? value.toString() : value
            ));
            res.json(itemResponse);
    ...
    public static async delete(req: Request, res: Response) {
        try {
            const { id } = req.params;
            await AccountHistoryService.delete(Number(id));
            res.json(true);
        } catch (error: any) {
            res.status(500).json(new ErrorResponseDto(undefined, error.message));
        }
